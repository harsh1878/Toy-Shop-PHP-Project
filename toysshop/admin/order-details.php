...
<html lang="en">
    <head>
        ...
        <title>Toys Store | Order Details</title>
...
<?php 
$oid=$_GET['orderid'];
$query=mysqli_query($con,"SELECT tbltoys.id as pid,tbltoys.ToyName,tbltoys.ToyImage1,tbltoys.ToyPriceAfterDiscount,tbltoys.ShippingCharge, ordersdetails.quantity FROM `ordersdetails` 
JOIN orders on orders.orderNumber=ordersdetails.orderNumber
join tbltoys on tbltoys.id=ordersdetails.productId
    where (orders.id='$oid')");
$cnt=1;
while($row=mysqli_fetch_array($query))
{
?>  
             ...
             <tr>
                    <td><img src="productimages/<?php echo htmlentities($row['ToyImage1']);?>" alt="<?php echo htmlentities($row['ToyName']);?>" width="100" height="100"></td>
                    <td>
                       <a href="edit-Toy.php?id=<?php echo htmlentities($pd=$row['pid']);?>" target="_blank"><?php echo htmlentities($row['ToyName']);?></a>
        </td>
<td><?php echo htmlentities($row['ToyPriceAfterDiscount']);?></td>
                    <td><?php echo htmlentities($row['quantity']);?></td>
                     <td><?php echo htmlentities($totalamount=$row['quantity']*$row['ToyPriceAfterDiscount']);?></td>
